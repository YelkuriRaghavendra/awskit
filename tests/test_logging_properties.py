...
class TestStructlogConfiguration:
    ...
    @given(
        ...
        queue_name=st.text(min_size=1, max_size=80, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd"), whitelist_characters="-_"
        )),
        message_text=st.text(min_size=1, max_size=200),
    )
    def test_log_output_is_valid_json(
        self, message_id: str, queue_name: str, message_text: str
    ):
        """
    ...
    def test_log_entries_contain_required_standard_fields(
        self, event_message: str, log_level: str
    ):
        """
...
class TestTemplateLogging:
    ...
    def setup_method(self):
        ...
        self.config = TemplateConfig(send_batch_failure_strategy=SendBatchFailureStrategy.DO_NOT_THROW)
    ...
    def test_message_operations_include_message_id_in_context(
        ...
        **Feature: structured-logging, Property 1: Message operations include message_id in context**
        ...
        self.client.get_queue_url.return_value = {"QueueUrl": f"https://sqs.us-east-1.amazonaws.com/123456789012/{queue_name}"}
        ...
        try:
            ...
            if log_output:
                ...
                if message_sent_log:
                    assert "message_id" in message_sent_log, "Log entry must contain message_id field"
        ...
        try:
            ...
            if log_output:
                ...
                if queue_operation_log:
                    assert "queue_name" in queue_operation_log, "Log entry must contain queue_name field"
                    assert "queue_url" in queue_operation_log, "Log entry must contain queue_url field"
    ...
    def test_batch_operations_include_batch_metadata_in_context(
        ...
        **Feature: structured-logging, Property 3: Batch operations include batch metadata in context**
        ...
        successful_items = [
            {"MessageId": f"msg-{i}", "SequenceNumber": None}
        ...
        try:
            ...
            if log_output:
                ...
                if batch_log:
                    assert "total_messages" in batch_log, "Log entry must contain total_messages field"
                    assert "successful_count" in batch_log, "Log entry must contain successful_count field"
...
class TestErrorLogging:
    ...
    @given(
        ...
        context_key=st.text(min_size=1, max_size=50, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd"), whitelist_characters="_"
        )),
        ...
        **Feature: structured-logging, Property 4: Error logs include exception information**
        ...
        try:
            ...
            if log_output:
                ...
                if error_log:
                    assert "exception" in error_log or "exc_info" in error_log, \
                    ...
                    assert "ValueError" in exception_info, \
                    ...
                    assert "Traceback" in exception_info or ".py" in exception_info, \
...
class TestContainerLogging:
    ...
    @given(
        message_id=st.text(min_size=1, max_size=100),
        queue_url=st.text(min_size=10, max_size=200, alphabet=st.characters(
            whitelist_categories=("Lu", "Ll", "Nd"), whitelist_characters="/:.-_"
        )),
            ...
            if log_output:
                ...
                for line in log_lines:
                    try:
                        ...
                        assert "message_id" in parsed, "Log entry must contain message_id from bound context"
                        assert "queue_url" in parsed, "Log entry must contain queue_url from bound context"
                        assert "receipt_handle" in parsed, "Log entry must contain receipt_handle from bound context"
            ...
            if log_output:
                ...
                for key, value in context_fields.items():
                    ...
                    if isinstance(value, float):
                        ...
                        assert abs(parsed[key] - value) < 0.0001, \
